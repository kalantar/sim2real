...
func (ws *WeightedScoring) Route(req *Request, state *RouterState) RoutingDecision {
	...
	// Fresh load signal for adaptive weighting
	...
	// Find best prefix-affinity instance
	bestPrefixID := ""
	bestPrefixScore := -1.0
	...
	// Adaptive weights: decay prefix-affinity when cached instance is overloaded
	...
	if bestPrefixID != "" && bestPrefixScore > 0.1 && len(ws.weights) >= 2 {
		...
		for _, snap := range snapshots {
			if snap.ID == bestPrefixID {
				cachedLoad = snap.InFlightRequests
				break
			}
		}
	...
	// Compute composite scores with adapted weights
	...
	// KV pressure penalty + fresh load tiebreaker
	for _, snap := range snapshots {
		if snap.KVUtilization > 0.9 {
			scores[snap.ID] -= 0.5 * (snap.KVUtilization - 0.9) / 0.1
		}
		// Small bonus for lower InFlightRequests to break ties with fresh data
